...
项目提供了**全新设计**的桌面图形界面，比之前的Web界面更稳定可靠！
...
相比之前的Streamlit Web界面：

| 对比项 | Streamlit版 | Tkinter版 (推荐) |
|--------|------------|-----------------|
| 稳定性 | ⚠️ 有多线程问题 | ✅ 高度稳定 |
| 依赖 | 需要streamlit等 | 仅需Python标准库 |
| 启动 | 需要浏览器 | 双击即用 |
| 实时性 | 需要刷新 | 真正实时输出 |

**详细使用指南**: 请查看 [`GUI_GUIDE.md`](GUI_GUIDE.md)
...
### v2.0.0 (2025-10-04) 🎉
- 🎨 **全新图形界面** - 基于Tkinter的稳定桌面GUI
- ✅ **完全重写** - 解决Web界面的所有稳定性问题
- ✅ **实时日志** - 彩色日志显示，真正的实时输出
- ✅ **简单易用** - 双击启动，无需配置浏览器
- ✅ **任务管理** - 启动、停止、进度显示一应俱全
- ✅ **零额外依赖** - 仅使用Python标准库
- 📚 **详细文档** - 新增 `GUI_GUIDE.md` 完整使用指南
- 🚀 **推荐使用** - 替代之前的Streamlit Web界面

### v1.0.4 (2025-10-04)
- 🐛 **修复Streamlit多线程问题** - 解决ScriptRunContext警告
- ✅ **改进错误处理** - 增强子进程调用的错误信息
- 📝 **技术文档** - 新增 `STREAMLIT_THREADING_FIX.md`
- ⚠️  **不推荐使用** - 建议使用v2.0的Tkinter界面

### v1.0.3 (2025-10-04)
- 🐛 **Web界面按钮修复** - 解决按钮禁用问题
- ✅ **简化版界面** - 创建无依赖版本
- 📚 **故障排除文档** - 新增 `WEB_TROUBLESHOOTING.md`

### v1.0.2 (2025-10-04)
- 🌐 **Web可视化界面** - 新增基于Streamlit的界面
- ⚠️  **已废弃** - 存在稳定性问题，已被v2.0替代
