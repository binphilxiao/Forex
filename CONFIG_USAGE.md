...
3. **rebuild_clickhouse_tables.py** - 重建数据库表
   - 读取：`config/clickhouse_config.json`
   - 默认：`config_path='config/clickhouse_config.json'`

4. **view_clickhouse_tables.py** - 查看数据库信息
   - 读取：`config/clickhouse_config.json`
   - 默认：`config_path='config/clickhouse_config.json'`

### 数据处理脚本（3个）
5. **fxcm_importer.py** - 数据导入器
   - 读取：`config/clickhouse_config.json`
   - 用途：自动获取数据库连接参数

6. **m1_timeframe_converter.py** - 时间框架转换器
   - 读取：`config/clickhouse_config.json`
   - 用途：数据库模式转换时使用

7. **verify_data_consistency.py** - 一致性验证
   - 读取：`config/clickhouse_config.json`
   - 默认：`config_path='config/clickhouse_config.json'`
